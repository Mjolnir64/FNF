...
{
	phx_radioNoteString pushBack format["<br/>BLOCK %1 CH %2", phx_152Chan, _forEachIndex + 1];
	if (_forEachIndex == 0) then {
		phx_radioNoteString pushBack " : Leader"
	} else {
		phx_radioNoteString pushBack format[" : Element %1", _forEachIndex];
	};
	if (_offset343 == _forEachIndex) then {
		phx_radioNoteString pushBack " (Your Group)"
	};
	phx_radioNoteString pushBack "</font>";
} forEach [1,2,3,4,5,6];
...
phx_radioNoteString pushBack "<br/><font size='20'>Your Pre-Tuned Channels</font>";
